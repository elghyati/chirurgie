 ...
 */
final class Version20200727125651 extends AbstractMigration
{
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, civilite, first_name, last_name, email, picture, hash, introduction, slug, job, company, adresse, ville, code_postal, enabled, tel, gsm, type FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, civilite VARCHAR(10) NOT NULL, first_name VARCHAR(25) NOT NULL, last_name VARCHAR(25) NOT NULL, email VARCHAR(50) NOT NULL, picture VARCHAR(255) DEFAULT NULL, hash VARCHAR(255) NOT NULL, introduction VARCHAR(255) DEFAULT NULL, slug VARCHAR(255) NOT NULL, job VARCHAR(255) NOT NULL, company VARCHAR(255) NOT NULL, adresse VARCHAR(255) NOT NULL, ville VARCHAR(255) NOT NULL, code_postal VARCHAR(10) DEFAULT NULL, enabled BOOLEAN DEFAULT NULL, tel VARCHAR(30) DEFAULT NULL, gsm VARCHAR(30) DEFAULT NULL, type VARCHAR(3) NOT NULL)');
        $this->addSql('INSERT INTO user (id, civilite, first_name, last_name, email, picture, hash, introduction, slug, job, company, adresse, ville, code_postal, enabled, tel, gsm, type) SELECT id, civilite, first_name, last_name, email, picture, hash, introduction, slug, job, company, adresse, ville, code_postal, enabled, tel, gsm, type FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
        $this->addSql('CREATE INDEX type_idx ON user (type)');
        $this->addSql('DROP INDEX IDX_2DE8C6A3A76ED395');
        $this->addSql('DROP INDEX IDX_2DE8C6A3D60322AC');
        $this->addSql('CREATE TEMPORARY TABLE __temp__user_role AS SELECT user_id, role_id FROM user_role');
        $this->addSql('DROP TABLE user_role');
